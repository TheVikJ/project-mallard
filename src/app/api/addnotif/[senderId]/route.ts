...
export async function POST(req: NextRequest, { params }: { params: Promise<{ senderId: string }> }) {
  ...
  try {
    ...
    switch (body.type) {
      case 'policy':
        res = await createPolicy(senderId, body.recipient, body.data);
        break;
      case 'claim':
        res = await createClaim(senderId, body.recipient, body.data);
        break;
      case 'news':
        res = await createNews(senderId, body.recipient, body.data);
        break;
      default:
...
async function createPolicy(senderId: string, recipientId: string, data: policy_notifs) {
...
async function createClaim(senderId: string, recipientId: string, data: claim_notifs) {
...
async function createNews(senderId: string, recipientId: string, data: news_notifs) {
